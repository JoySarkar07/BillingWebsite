    ...
    <div className="receipt-popup-overlay text-dark">
      <div className="receipt-popup">
        ...
        <div className="cart-items-scrollable">
            {
              orderDetails.items.map((item, ind)=>{
                return <div key={ind} className="d-flex justify-content-between mb-2">
                    <span>{item.name} X {item.quantity}</span>
                    <span><i className="bi bi-currency-rupee"></i>{(item.price * item.quantity).toFixed(2)}</span>
                </div>
              })
            }
        </div>
        ...
        <div className="d-flex justify-content-between mb-2">
            ...
            <span><i className="bi bi-currency-rupee"></i>{orderDetails.subtotal.toFixed(2)}</span>
        </div>
        <div className="d-flex justify-content-between mb-2">
            <span>
              <strong>Tax (1%):</strong>
            </span>
            <span><i className="bi bi-currency-rupee"></i>{orderDetails.tax.toFixed(2)}</span>
        </div>
        <div className="d-flex justify-content-between mb-2">
            <span>
              <strong>Grand Total:</strong>
            </span>
            <span><i className="bi bi-currency-rupee"></i>{orderDetails.grandTotal.toFixed(2)}</span>
        </div>
        <p>
